...
describe("shared.js: sendMessage failure shape", () => {
  test("returns { error, code: 'no_chrome' } when chrome runtime is unavailable", async () => {
    ...
    expect(resp).toEqual({
      error: expect.stringMatching(/chrome runtime unavailable/),
      code: "no_chrome"
    });
  });

  test("returns { error, code: 'send_failed' } when chrome.runtime.lastError is set", async () => {
    // The mock is shared with the IIFE closure; setting lastError on the same
    // object before invoking the callback mirrors what chrome does in MV3.
    const stub = { runtime: { lastError: null, sendMessage: null } };
    stub.runtime.sendMessage = (msg, cb) => {
      stub.runtime.lastError = {
        message: "Could not establish connection. Receiving end does not exist."
      };
      try { cb(undefined); } finally { stub.runtime.lastError = null; }
    };
    const gcc = buildGcc(stub);
    const resp = await gcc.sendMessage({ type: "ping" });
    expect(resp).toEqual({
      error: expect.stringMatching(/Could not establish connection/),
      code: "send_failed"
    });
  });

  test("returns { error, code: 'threw' } when chrome.runtime.sendMessage throws", async () => {
    const gcc = buildGcc({
      runtime: {
        lastError: null,
  ...
  test("passes the service worker's reply through unchanged on success", async () => {
    const reply = { ok: true, stats: { runs: 3 } };
    const gcc = buildGcc({
      runtime: {
        lastError: null,
        sendMessage: (msg, cb) => cb(reply)
      }
    });
    const resp = await gcc.sendMessage({ type: "ping" });
    expect(resp).toBe(reply);
  });
});
