...
- **One button erases what it learned about your contacts** - the scans build an on-device index of who emails you, and Options has an Erase Stored Sender Data button that removes every store holding an address in one write: the sender census, your unsubscribe receipts, the ticked-sender lists, the mailbox report, the storage X-ray, the suggestion and subscription scans, the record of which suggestions you approved or dismissed, the top senders list on the Stats page, and the searches your past runs used. The four scans go back to asking for a scan and your run counts and totals stay. Your rules, whitelist, schedules, Pro key and recovery log are settings rather than mailbox data, and are left alone, so anything waiting in Trash can still be restored. Diagnostics shows the counts, never the addresses, so you can check the button did what it said.
